...
* GetClientDims
*/
IntVec2 WindowContext::GetClientDims() const
{
	RECT clientRect; 
	GetClientRect( (HWND)m_hwnd, &clientRect ); 

	IntVec2 dims; 
	dims.x = (clientRect.right - clientRect.left); 
	dims.y = (clientRect.bottom - clientRect.top); 
...
* GetClientRect
*/
AABB2 WindowContext::GetClientScreen() const
{
	RECT clientRect; 
	GetClientRect( (HWND)m_hwnd, &clientRect ); 

	return AABB2( (float) clientRect.left, (float) clientRect.bottom, (float) clientRect.right, (float) clientRect.top );
}
