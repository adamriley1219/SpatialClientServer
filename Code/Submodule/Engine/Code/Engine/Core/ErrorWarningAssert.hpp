...
NO_RETURN void FatalError( const char* filePath, const char* functionName, int lineNum, const std::string& reasonForError, const char* conditionText=nullptr );
...
#if defined( DISABLE_ASSERTS )
#define ASSERT_RETURN_VALUE( condition, errorMessageText, returnThis ) {  (void)( condition );  (void)( returnThis ); }
#else
#define ASSERT_RETURN_VALUE( condition, errorMessageText, returnThis )								\
{																									\
	if( !(condition) )																				\
	{																								\
		const char* conditionText = #condition;														\
