...
* Startup
*/
void SpatialOSServer::Startup( const std::vector<std::string>& args )
{
	GetInstance()->server_thread = std::thread( Run, args );
}
...
*/
void SpatialOSServer::RequestEntityCreation( const worker::Entity& entity )
{
	if( !IsRunning() )
	{
		return;
