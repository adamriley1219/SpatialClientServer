...
//--------------------------------------------------------------------------------------
cbuffer game_buffer : register(b8) 
{
	float VAR1;
	float VAR2;
	float VAR3;
...
Texture2D<float4> tNoise : register(t4); 
SamplerState sNoise : register(s4);  
...
float4 FragmentFunction( v2f_t input ) : SV_TARGET0
{
	...
	float4 noiseValue = tNoise.Sample( sNoise, input.uv );


	clip( noiseValue.x - VAR1 );
	...
	return lerp( float4( 1.0f, 0.0f, 0.0f, 1.0f ), final_color, clamp( ( noiseValue.x - VAR1 ) / 0.05f, 0.0f, 1.0f ) );
